...
public class CustomAuthorizer extends AuthorizingRealm implements Serializable {   
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        if (user != null) {
           ...
           for (Role role : roles) {
               List<Permission> permissions = null; // loginService.findAllPermissionByRole(role);
    ...
    public AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
        ...
        if (user != null) {
            return null; // new SimpleAuthenticationInfo(user.getLoginName(), user.getUserPass(), getName());
        } else {
            return null;
        }
    }
