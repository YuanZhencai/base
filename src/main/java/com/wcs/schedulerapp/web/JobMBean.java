...
public class JobMBean implements java.io.Serializable {
	...
	/*
	 * Action handler for New Job button 行动处理器新工作的按钮
	 */
	public String gotoNew() {
		System.out.println("been   gotoNew() called!!!");
		newJob = new JobInfo();
		return "JobNew";
	}

	/*
	 * Action handler for Duplicate button in the Details page 行动处理器重复按钮的详细页面
	 */
	...
	/*
	 * Action handler for Update button in the Details page 行动处理器更新按钮的详细页面
	 */
	public String updateJob() {
		FacesContext context = FacesContext.getCurrentInstance();
		try {
			System.out.println("~~~~~~~~~~~~update  been~~~~~~~~~~~~~");
			selectedJob = jobSessionBean.updateJob(selectedJob);
			System.out.println("~~~~~~~~~~~~update  been  完成~~~~~~~~~~~~~");
			context.addMessage(null, new FacesMessage(
					FacesMessage.SEVERITY_INFO, "Success",
					"Job successfully updated!"));
		} catch (Exception ex) {
			Logger.getLogger(JobMBean.class.getName()).log(Level.SEVERE, null,
					ex);
			context.addMessage(null, new FacesMessage(
	...
	public String createJob() {
		...
		try {
			...
			System.out.println("~~~~~~~~~~创建定时器完成~~~~~~~~~~~");
			context.addMessage(null, new FacesMessage(
					FacesMessage.SEVERITY_INFO, "Sucess",
