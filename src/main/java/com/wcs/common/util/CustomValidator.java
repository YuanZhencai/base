...
//		if((regexMarkArray.length !=0 && regexMarkArray.length !=regexArray.length)||(regexMessageArray.length !=0 && regexMessageArray.length !=regexArray.length) ){
//			throw new ValidatorException(new FacesMessage(
//					FacesMessage.SEVERITY_ERROR,
//					"请核对多个正则验证属性是否一一对应!(##隔开.)",""));
//		}
		...
		if (regex !=null) {
			...
			for(int i=0;i<regexArray.length;i++){
				...
				if(regexMarkArray[i].toString().equals("none")){
